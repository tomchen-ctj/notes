...
Existing methods for video contrastive learning naively sample different clips to construct pos&neg pairs. But this may lead to background bias. Different clips from same video may have the same static background but distinct motions. The authors propose a dual contrastive formulation, that decouples the video into static scene and motion patterns. Take static repeating random frame as static data and regard the frame difference as the dynamic data. The model is trained by respectively minimizing two dual contrastive losses. One is the alignment between the original RGB sequence and static input, the other is between RGB and dynamic input. 

Static-dynamic decoupling in data input: repeat a randomly selected frame to carry the static information without any dynamic motion. And use frame difference as dynamic information. To prevent the collapse of video encoder, add another regularization term to ensure disparity between fs and fd. But the positive pair often shares similar backgrounds but differs in motions, the dynamic motions do not always align. Retrieve the sample with highest similarity score and use this as positive pairs.  

The decoupling is further enhanced in the latent space by activation maps. The activation map of video encoder should jointly highlight the static and dynamic information areas. This could filter out the possible noise that may interfere with the dual contrastive objective and enhance the representation ability. [link](https://arxiv.org/pdf/2207.05340.pdf)

## IS APPEARANCE FREE ACTION RECOGNITION POSSIBLE?

Propose an extension to a widely used action recognition dataset that disentangles static and dynamic video components. The dataset is generated by optical flow and reference warping. [link](https://arxiv.org/pdf/2207.06261)

## Efficient Human Vision Inspired Action Recognition using Adaptive Spatiotemporal Sampling

Inspired by the concepts of foveal vision and pre-attentive processing from the human visual perception mechanism, pre-scans the global scene context at low-resolution and decides to skip or request high-resolution features at salient regions for further processing. 

The temporal sampler decides whether to process the frame fully or skip to the frame and propagate past information. The spatial sampler in turns select RoIs from high-res input to augment the features with low-res inputs. Hallucinate future attention and compare it with the observed one, when the hallucination matches the actual attention, there is no unexpected event and the model simply uses the previous classification results. (Using SSIM as metric to determine) [link](https://arxiv.org/pdf/2207.05249.pdf)
